...
public class ModGenerator {
    ...
    /** Converts a display name to a valid Java class name (PascalCase, letters/digits only). */
    static String toJavaClassName(String name) {
        ...
        for (char c : name.toCharArray()) {
            if (Character.isLetterOrDigit(c)) {
                ...
                capitalizeNext = true; // capitalize after any separator
            }
        }
    ...
    /** Converts a display name to a safe registry/resource id (lowercase, underscores). */
    static String toSafeId(String name) {
    ...
    private static void writeJavaClasses(File dir, String group, String modId, String modName, List<ModElement> elements, boolean useCustomCreativeTab) throws IOException {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (el.isBlock()) {
                ...
                String entityClassName = toJavaClassName(ed.getName()) + "Entity";
        ...
        for (int i = 0; i < elements.size(); i++) {
            if (elements.get(i).isEntity()) {
                String constName = "E_" + i;
                String entityClassName = toJavaClassName(elements.get(i).getName()) + "Entity";
            ...
            for (ModElement el : elements) {
                if (el.isBlock()) {
                    ...
                    if (data.getToolType().equals(tool)) {
                        ...
                        tagValues.append("    \"").append(modId).append(":").append(toSafeId(data.getName())).append("\"");
                        hasEntries = true;
                    }
                }
            }
    ...
    private static void createRecipes(File workDir, String modId, List<ModElement> elements) throws IOException {
        ...
        for (ModElement el : elements) {
            ...
            String nameId = toSafeId(el.getName());
